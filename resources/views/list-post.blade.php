@section('page-title')<?php echo $position;?>@endsection
@section('page-description')<?php echo 'timnhacungban.com sẽ cung cấp cho các bạn sinh viên những tin nhà trọ mới nhất, chính xác nhất và hoàn toàn miễn phí '.$position;?>@endsection
@include('header')
    ...
    <div class="container no-padding">
        <div class="row">
            <div class="col-xs-12 col-sm-9 col-md-9 content-left">
                <h3 class="full-width entry-title"><?php echo $position;?></h3>
                ...
                @if(count($value) <= 10 && !empty($value))
                    <div class="col-xs-12 text-center pagination">
                        ...
                        ?>
                        @if($page != 1)
                            <a class="btn btn-default" href="<?php echo $root_url;?>" data-page="1" title="Trang đầu tiên"> <i
                                        class="fa fa-angle-double-left" aria-hidden="true"></i> </a>
                            <a class="btn btn-default" href="<?php echo $root_url;?>&page={{$page - 1}}" title="Trang trước"
                        ...
                        @for ($i = $min ; $i <= $max; $i++)
                            @if($i <= $total && $i > 0)
                                <a class="btn btn-default <?php if($page == $i) { ?> active <?php } ?>"
                                   href="<?php echo $root_url;?>&page={{$i}}" data-page="{{$i}}" title="Trang {{$i}}">
                                    {{$i}}
                                </a>
                            @endif
                        @endfor
                        ...
                        @if($page != $total)
                            <a class="btn btn-default" href="<?php echo $root_url;?>&page={{$page + 1}}"
                               data-page="{{$page + 1}}" title="Trang tiếp theo"> <i class="fa fa-angle-right"
                                                                                     aria-hidden="true"></i> </a>
                            <a class="btn btn-default" href="<?php echo $root_url;?>&page={{$total}}"
